...
SUBMODULE_BRANCH="oadp-1.3"
SUBMODULE_URL="https://github.com/openshift/velero"
SUBMODULE_PATH="velero"
...
stage_and_commit(){
    ...
    if [[ -n $(git status --porcelain) ]]; then
        ...
        git commit "${author_flag[@]}" -q \
          -m "UPSTREAM: <drop>: update velero @ ${submodule_short_hash} (branch $SUBMODULE_BRANCH)"
    fi
}
