...
stage_and_commit(){
    ...
    if [[ -n $(git status --porcelain) ]]; then
        git add -A
        git commit "${author_flag[@]}" -q -m "UPSTREAM: <drop>: Use new dynamic text"
    fi
}

echo $(date) >> ./rebasebot-new-text.txt

stage_and_commit

echo "Dynamic file ./rebasebot-new-text.txt updated to the latest."
